...
This benchmarks the CPU for 30 seconds, then the GPU for 30 seconds, and saves JSON and PNG results without survey prompts. The PNG contains separate scores, thermal timelines and test statuses. Saved paths appear in the terminal. Press Ctrl+C to stop early and save partial results.
...
Try a sample PNG report without running a benchmark:

```powershell
.\thermal.exe demo --png demo.png
Invoke-Item .\demo.png
```
...
This saves **before.json** (measurements) and **before.png** (visual report).
...
.\thermal.exe compare before.json after.json --png comparison.png
...
```powershell
.\thermal.exe report after.json --png report.png
```

Files are never overwritten. Choose a new filename when repeating these examples.
Use `--png custom.png` to choose the PNG path, or `--no-png` to save only JSON during `record`, `benchmark`, or `import`.
...
```sh
go run ./cmd/thermal demo --png demo.png
```
