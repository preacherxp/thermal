...
func (g *openCLGPU) call(name string, args ...uintptr) uintptr {
	return g.library.call(name, args...)
}
...
func newGPUBackend() (_ gpuBackend, err error) {
	library, err := loadOpenCL()
	if err != nil {
		return nil, err
	}
	g := &openCLGPU{library: library}
	defer func() {
		if err != nil {
			g.Close()
		}
...
var openCLSymbols = []string{"clGetPlatformIDs", "clGetDeviceIDs", "clGetDeviceInfo", "clCreateContext", "clCreateCommandQueue", "clCreateProgramWithSource", "clBuildProgram", "clGetProgramBuildInfo", "clCreateKernel", "clCreateBuffer", "clSetKernelArg", "clEnqueueNDRangeKernel", "clEnqueueReadBuffer", "clReleaseMemObject", "clReleaseKernel", "clReleaseProgram", "clReleaseCommandQueue", "clReleaseContext"}
